...
| 10 | How do we see cost and behaviour? | kagent emits [OpenTelemetry traces and Prometheus metrics](https://kagent.dev/docs/kagent/observability/tracing) and can ship [every prompt and tool call to a SIEM](https://kagent.dev/docs/kagent/observability/audit-prompts/). agentgateway traces to Langfuse. LiteLLM reports spend per key | All three docs. Our test: one prompt agent request produced `invoke_agent`, `generate_content` with token counts, and `execute_tool` with the tool name in Tempo | Wired and tested for traces. The OpenTelemetry Collector receives kagent traces and audit logs, agentgateway spans and access logs with `jwt_sub` and `gen_ai_tool_name` on every line, LiteLLM spans, and the template app's own spans. It scrapes LiteLLM spend per key and team, agentgateway request counters, and kagent controller metrics. kagent's prompt audit stream lands in Loki with content elided, because content capture is off. One gap: the kagent Go runtime does not pass the trace header to LiteLLM, so for prompt agents cost sits in a separate trace joined by time only. Container agents do pass it. No message content is captured |
