...
| 10 | How do we see cost and behaviour? | kagent emits [OpenTelemetry traces and Prometheus metrics](https://kagent.dev/docs/kagent/observability/tracing) and can ship [every prompt and tool call to a SIEM](https://kagent.dev/docs/kagent/observability/audit-prompts/). agentgateway traces to Langfuse. LiteLLM reports spend per key | All three docs | Wired. One collector pod receives kagent traces and audit logs, agentgateway spans and access logs, LiteLLM spans and spend metrics, and the template app's own spans. One request is one trace across all four. No message content is captured. Not yet tested on a running cluster |
