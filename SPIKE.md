...
| # | Concern | The mechanism, and the tool that provides it | Evidence | In this proof of concept |
|---|---|---|---|---|
| 13 | Who may provision what, and at whose expense? | LiteLLM has [organizations and teams](https://docs.litellm.ai/docs/proxy/users) that each carry their own `max_budget`. A key created with a `team_id` spends against the team, so a team cannot pass its own cap however many agents it creates. Argo CD AppProjects decide who may create an agent at all | LiteLLM docs. Our test: Argo refused a foreign namespace and a foreign repo | Keys carry team, owner, and agent as metadata, so spend attributes correctly. No team object and no team cap, so per-agent caps never sum. Nobody signs for the total |
...
| 16 | Deleting the agent must end the obligation | LiteLLM `/key/delete` removes a key, and a key created with a `duration` expires by itself. agent-sandbox does [automatic cleanup after a configurable TTL](https://agent-sandbox.sigs.k8s.io/docs/) and can hibernate an idle sandbox. Argo CD prunes whatever leaves Git | LiteLLM and agent-sandbox docs. Our test: deleting a folder removed the workload, the ConfigMaps, and the minted Secret | The garbage collector in `platform/minter/minter.py` deletes the Secret and the gateway policy but never calls `/key/delete`, so a budgeted key stays valid. Keys get no `duration`. Sandboxes get no TTL |
...
The gaps in the last column are configuration plus one small code change, not
research. Rows 16 and 13 are the two I would close first, because both leave
money live after the project that spent it is gone.
