...
| 15 | Agents pile up keys and tokens across many services | agentgateway's [backend authentication](https://agentgateway.dev/docs/standalone/latest/configuration/security/backend-authn/) holds one credential per target and injects it on the way out. It supports a static key, a signed JWT, cloud provider credentials, and OAuth token exchange, and reads secrets from a Kubernetes `secretRef`. The agent presents its own identity and never holds the third-party secret. External Secrets Operator can source those secrets from a vault that rotates them | agentgateway docs | Wired for the worked example. The Orders API demands a key, returns 401 to a direct caller, and the agent still reads orders because the gateway injects it. The MCP wrapper's only variable is a URL. `envFrom` stays as an escape hatch, and the dev guide states what it costs: no rotation, no scoping, no inventory. Token exchange and per-agent identity are available and unused |
...
The gaps in the last column are configuration, not research. Row 14 is the one
I would close next. Nothing reaches an outside service yet, and the `egress`
field promises something it does not deliver.
