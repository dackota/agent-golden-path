...
For every agent registration ConfigMap (label platform.suresteel.com/registration=true):
...
LABEL = "platform.suresteel.com/registration=true"
MINTED = {"platform.suresteel.com/minted": "true"}
...
def mint_jwt(sub, days=30):
    ...
    payload = b64url(json.dumps({"iss": "platform.suresteel.com", "aud": "platform-tools", "sub": sub,
...
def ensure_secret(reg, ns):
    ...
    body = {"apiVersion": "v1", "kind": "Secret", "type": "Opaque",
            "metadata": {"name": name, "namespace": ns, "labels": {**MINTED, "platform.suresteel.com/agent-name": reg["name"]}},
            "stringData": data}
...
def ensure_policy(reg, ns):
    ...
    body = {"apiVersion": "agentgateway.dev/v1alpha1", "kind": "AgentgatewayPolicy",
            "metadata": {"name": name, "namespace": GATEWAY_NS, "labels": {**MINTED, "platform.suresteel.com/agent-ns": ns}},
...
def ensure_ns_label(ns):
    req = urllib.request.Request(f"{API}/api/v1/namespaces/{ns}", method="PATCH",
                                 data=json.dumps({"metadata": {"labels": {"platform.suresteel.com/agent-namespace": "true"}}}).encode(),
...
def gc(live):
    ...
    _, secrets = k8s("GET", "/api/v1/secrets?labelSelector=platform.suresteel.com/minted=true")
    for s in secrets.get("items", []):
        key = (s["metadata"]["namespace"], s["metadata"]["labels"].get("platform.suresteel.com/agent-name"))
        if key not in live:
            k8s("DELETE", f"/api/v1/namespaces/{key[0]}/secrets/{s['metadata']['name']}")
            print(f"gc secret {key[0]}/{s['metadata']['name']}")
    _, pols = k8s("GET", f"/apis/agentgateway.dev/v1alpha1/namespaces/{GATEWAY_NS}/agentgatewaypolicies?labelSelector=platform.suresteel.com/minted=true")
    for p in pols.get("items", []):
        ns = p["metadata"]["labels"].get("platform.suresteel.com/agent-ns")
